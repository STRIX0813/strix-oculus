...
def format_market_cap_server(cap_raw: Any) -> str:
    if not cap_raw:
        return '-'
    cap_str = str(cap_raw).strip()
    if re.match(r'^\$?\s*\d+(\.\d+)?[TBM]$', cap_str, re.I):
        return cap_str if cap_str.startswith('$') else f"${cap_str}"
    if '$' in cap_str:
        try:
            num = float(re.sub(r'[^\d.-]', '', cap_str))
    ...
    for page in range(1, 15):
        ...
        try:
            ...
            for item in d_us.get('stocks', []):
                ...
                raw_cap = item.get('marketValue', None)
                if raw_cap:
                    try:
                        # Naver overseas API reports marketValue in Thousand USD (* 1000)
                        cap_dollars = float(str(raw_cap).replace(',', '')) * 1000.0
                        if cap_dollars >= 1_000_000_000_000:
                            cap_str = f"${cap_dollars / 1_000_000_000_000:.1f}T"
                        elif cap_dollars >= 1_000_000_000:
                            cap_str = f"${cap_dollars / 1_000_000_000:.1f}B"
                        elif cap_dollars >= 1_000_000:
                            cap_str = f"${cap_dollars / 1_000_000:.1f}M"
                        else:
                            cap_str = f"${cap_dollars:,.0f}"
                    except Exception:
                        cap_str = format_market_cap_server(raw_cap)
                else:
                    cap_str = '-'
